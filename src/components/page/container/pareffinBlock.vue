<template>
    <div>
        <div class="headerTop">
            ...
            <div class="headerTop_text">石蜡块柜管理</div>
        </div>
        <!-- 头部搜索条件 -->
        ...
        <div class="container1">
            <div class="item" v-for="item in tableData" :key="item.containerid">
                <div class="item1 kk">
                    <div class="itemTop">
                        <img src="../../../assets/img/paraffin.png" alt />
                    </div>
                    <div class="itemBot">
                        <p>{{item.name}}</p>
                    </div>
                ...
                <div class="item2 kk">
                    ...
                    <div class="itemBot2">
                        <div class="title2">石蜡块柜可放入材料：</div>
                        <!-- <div class="txt2">石蜡块柜可放入材料石蜡块柜可放入材料石蜡块柜可放入材料石蜡块柜可放入材料石蜡块柜可放入材料石蜡块柜可放入材料</div> -->
                    </div>
        ...
        <!-- 添加石蜡块柜 -->
        <el-dialog
            :title="addForm.id?'编辑石蜡块柜':'新增石蜡块柜'"
        ...
        >
            <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="90px">
                ...
                <el-form-item label="石蜡块柜编号" prop="no">
                    <el-input type="text" v-model="addForm.no"></el-input>
                </el-form-item>
                <el-form-item label="石蜡块柜名称" prop="name" :status-icon="true">
                    <el-input type="text" v-model="addForm.name"></el-input>
                </el-form-item>
                <el-form-item label="石蜡块柜品牌" prop="brand" :status-icon="true">
                    <el-input type="text" v-model="addForm.brand"></el-input>
                </el-form-item>
                <el-form-item label="石蜡块柜型号" prop="model" :status-icon="true">
    ...
    data() {
        return {
            ...
            addFormRules: {
                ...
                no: [{ required: true, trigger: 'blur', message: '请输入石蜡块柜编号' }],
                name: [{ required: true, trigger: 'blur', message: '请输入石蜡块柜名称' }],
                brand: [{ required: true, trigger: 'blur', message: '请输入石蜡块柜品牌' }],
                model: [{ required: true, trigger: 'blur', message: '请输入石蜡块柜型号' }],
    ...
    methods: {
        ...
        handleEdit(item) {
            ...
            this.addForm = {
                ...
                no: item.no, //石蜡块柜编号
                name: item.name, //石蜡块柜名称
                brand: item.brand, //石蜡块柜品牌
        ...
        getData(pageIndex) {
            this.$http
                .post('/api/container/getlistbyarchives', {
                    name: this.keyword,
                    roomid: this.value,
                    pageIndex: pageIndex,
        ...
        addSubmitForm(formName) {
            this.$refs.addFormRef.validate((valid) => {
                if (valid) {
                    ...
                    if (this.addForm.id) {
                        this.$http
                            .post('/api/container/savebyarchives', {
        ...
        // 删除
        handleDelete(item) {
            this.$confirm('此操作将永久删除该石蜡块柜, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消'
            })
                .then(() => {
                    this.$http.post('/api/container/deletebyarchives', { containerid: item.containerid }).then((res) => {
                        if (res.data.success) {
                            this.$message({
                                type: 'success',
                                message: '删除成功!'
